...
function FullscreenTrapezoid({ topBase, bottomBase, height, unitSymbol, isValid, onClose, onSizeChange }) {
    ...
    const [trapezoidFillColor, setTrapezoidFillColor] = useState('gradient');
    ...
    useEffect(() => {
        ...
        if (isValid && trapezoidData) {
            ...
            if (trapezoidFillColor === 'gradient') {
                ...
                ctx.fillStyle = trapezoidFillColor + '80';
            }
            ctx.fill();
        ...
        <div className="fullscreen-whiteboard" ref={containerRef}>
            ...
            <div className={`whiteboard-toolbar ${isToolbarOpen ? 'open' : ''}`}>
                ...
                <div className="toolbar-section fill-section">
                    <span className="fill-label">Trapetsiya:</span>
                    <div className="fill-buttons">
                        <button className={`fill-btn ${trapezoidFillColor === 'gradient' ? 'active' : ''}`} onClick={() => setTrapezoidFillColor('gradient')} title="Gradient">
                            <span className="gradient-preview"></span>
                        </button>
                        {COLOR_PALETTE.slice(0, 5).map(color => (
                            <button key={`fill-${color}`} className={`fill-btn ${trapezoidFillColor === color ? 'active' : ''}`} style={{ backgroundColor: color }} onClick={() => setTrapezoidFillColor(color)} title={`Bo'yash: ${color}`} />
                        ))}
                    </div>
                </div>
                <div className="toolbar-divider" />

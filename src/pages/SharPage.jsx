...
// 3D Shar komponenti
function Shar3D({ radius, tiltAngle, showWireframe }) {
...
export function SharPage() {
    ...
    const calculations = useMemo(() => {
        ...
        // Yarim shar hajmi
        const hemisphereVolume = volume / 2;

        // Yarim shar sirt maydoni (asossiz)
        const hemisphereSurface = 2 * Math.PI * r * r;
        ...
        return {
            ...
            hemisphereVolume: hemisphereVolume.toFixed(2),
            hemisphereSurface: hemisphereSurface.toFixed(2)
        };
    }, [radius]);
        ...
        <div className="prizma-page">
            ...
            <header className="prizma-header">
                ...
                <div className="header-title">
                    <h1>● Shar</h1>
                    <p>Shar (sfera) - professional modellashtirish</p>
                </div>
                <UserMenu />
            </header>
                ...
                {isFullscreen && (
                    <div className="fullscreen-3d-modal" ref={canvasContainerRef}>
                        ...
                        <Canvas shadows style={{ background: 'transparent' }}>
                            ...
                            <pointLight position={[-10, -10, -10]} intensity={0.3} color="#60a5fa" />
                            <Environment preset="city" />
                            <group scale={renderScale}><Shar3D radius={radius} tiltAngle={tiltAngle} showWireframe={showWireframe} /></group>
                        ...
                        <div className="fullscreen-top-bar">
                            <div className="top-bar-left"><span className="brand-text">● Shar 3D</span></div>
                            ...
                            <div className="panel-content">
                                ...
                                <div className="panel-section">
                                    ...
                                    <div className="calc-mini-grid">
                                        ...
                                        <div className="calc-mini-item"><span className="calc-mini-label">Sirt maydoni</span><span className="calc-mini-value">{calculations.surfaceArea} {unitSymbol}²</span></div>
                                        <div className="calc-mini-item"><span className="calc-mini-label">Diametr</span><span className="calc-mini-value">{calculations.diameter} {unitSymbol}</span></div>
                                        <div className="calc-mini-item"><span className="calc-mini-label">Aylana</span><span className="calc-mini-value">{calculations.circumference} {unitSymbol}</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <aside className="formulas-panel">
                    ...
                    <div className="calc-section">
                        ...
                        <div className="calc-grid">
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Hajmi (V)', value: `${calculations.volume} ${unitSymbol}³`, formula: 'V = (4/3)πr³', description: `V = (4/3) × π × ${radius}³ = ${calculations.volume} ${unitSymbol}³` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Hajmi (V)</span>
                                <span className="calc-value">{calculations.volume} {unitSymbol}³</span>
                            </div>
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Sirt maydoni (S)', value: `${calculations.surfaceArea} ${unitSymbol}²`, formula: 'S = 4πr²', description: `S = 4 × π × ${radius}² = ${calculations.surfaceArea} ${unitSymbol}²` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Sirt maydoni (S)</span>
                                <span className="calc-value">{calculations.surfaceArea} {unitSymbol}²</span>
                            </div>
                            ...
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Aylana uzunligi', value: `${calculations.circumference} ${unitSymbol}`, formula: 'C = 2πr', description: `C = 2 × π × ${radius} = ${calculations.circumference} ${unitSymbol}` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Aylana uzunligi</span>
                                <span className="calc-value">{calculations.circumference} {unitSymbol}</span>
                            </div>
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Katta doira yuzasi', value: `${calculations.greatCircleArea} ${unitSymbol}²`, formula: 'S = πr²', description: `S = π × ${radius}² = ${calculations.greatCircleArea} ${unitSymbol}²` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Katta doira yuzasi</span>
                                <span className="calc-value">{calculations.greatCircleArea} {unitSymbol}²</span>
                            </div>
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Yarim shar hajmi', value: `${calculations.hemisphereVolume} ${unitSymbol}³`, formula: 'V = (2/3)πr³', description: `V = (2/3) × π × ${radius}³ = ${calculations.hemisphereVolume} ${unitSymbol}³` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Yarim shar hajmi</span>
                                <span className="calc-value">{calculations.hemisphereVolume} {unitSymbol}³</span>
                            </div>
                            <div className="calc-item clickable" onClick={() => { setSelectedCalc({ label: 'Yarim shar sirti', value: `${calculations.hemisphereSurface} ${unitSymbol}²`, formula: 'S = 2πr²', description: `S = 2 × π × ${radius}² = ${calculations.hemisphereSurface} ${unitSymbol}²` }); setShowCalcModal(true); }}>
                                <span className="calc-label">Yarim shar sirti</span>
                                <span className="calc-value">{calculations.hemisphereSurface} {unitSymbol}²</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="formulas-section">
                        ...
                        <div className="formula-card">
                            ...
                            <div className="formula-eq">V = (4/3)πr³</div>
                            <div className="formula-desc">
                                V = (4/3) × π × {radius}³ = {calculations.volume} {unitSymbol}³
                            </div>
                        </div>

                        <div className="formula-card">
                            <div className="formula-title">Sirt maydoni</div>
                            <div className="formula-eq">S = 4πr²</div>
                            <div className="formula-desc">
                                S = 4 × π × {radius}² = {calculations.surfaceArea} {unitSymbol}²
                            </div>
                        </div>

                        <div className="formula-card">
                            <div className="formula-title">Diametr</div>
                            <div className="formula-eq">d = 2r</div>
                            <div className="formula-desc">
                                d = 2 × {radius} = {calculations.diameter} {unitSymbol}
                            </div>
                        </div>

                        <div className="formula-card">
                            <div className="formula-title">Katta doira uzunligi</div>
                            <div className="formula-eq">C = 2πr</div>
                            <div className="formula-desc">
                                C = 2 × π × {radius} = {calculations.circumference} {unitSymbol}
                            </div>
                        </div>

                        <div className="formula-card">
                            <div className="formula-title">Katta doira yuzasi</div>
                            <div className="formula-eq">S<sub>doira</sub> = πr²</div>
                            <div className="formula-desc">
                                S = π × {radius}² = {calculations.greatCircleArea} {unitSymbol}²
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="properties-section">
                        ...
                        <ul className="properties-list">
                            <li>Markazdan barcha nuqtalarga teng masofa — radius</li>
                            <li>Shar — eng katta hajmga ega shakl (berilgan sirt uchun)</li>
                            <li>Ekvatorial kesim — katta doira</li>
                            <li>Cheksiz ko'p simmetriya o'qlari</li>
                            <li>Aylantirish jismi — yarim doirani aylantirishda hosil bo'ladi</li>
                        </ul>
                    </div>
                </aside>
            </div>

...
// 3D Shar komponenti
function Shar3D({ radius, tiltAngle, showWireframe, fillColor }) {
    ...
    const materialColor = fillColor === 'gradient' ? '#8b5cf6' : fillColor;
...
export function SharPage() {
    ...
    const [sharFillColor, setSharFillColor] = useState('gradient');
    ...
    const saveDrawing = () => {
        ...
        link.download = `shar-chizma-${new Date().getTime()}.png`;
                    ...
                    <div className="fullscreen-whiteboard" ref={canvasContainerRef}>
                        ...
                        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
                            <Canvas shadows>
                                ...
                                <Environment preset="city" />
                                <group scale={renderScale}>
                                    <Shar3D radius={radius} tiltAngle={tiltAngle} showWireframe={showWireframe} fillColor={sharFillColor} />
                                </group>
                                {showGrid && (
                        ...
                        <div className={`whiteboard-toolbar ${isToolbarOpen ? 'open' : ''}`}>
                            ...
                            <div className="toolbar-section fill-section">
                                <span className="fill-label">Shar:</span>
                                <div className="fill-buttons">
                                    <button className={`fill-btn ${sharFillColor === 'gradient' ? 'active' : ''}`} onClick={() => setSharFillColor('gradient')} title="Gradient">
                                        <span className="gradient-preview"></span>
                                    </button>
                                    {COLOR_PALETTE.slice(0, 5).map(color => (
                                        <button key={`fill-${color}`} className={`fill-btn ${sharFillColor === color ? 'active' : ''}`} style={{ backgroundColor: color }} onClick={() => setSharFillColor(color)} />
                                    ))}
                                </div>
                            </div>
                            <div className="toolbar-divider" />

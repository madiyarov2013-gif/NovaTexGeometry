...
function Silindr3D({ radius, height, tiltAngle, showWireframe, fillColor }) {
    ...
    const materialColor = fillColor === 'gradient' ? '#f59e0b' : fillColor;
...
export function SilindrPage() {
    ...
    const [silindrFillColor, setSilindrFillColor] = useState('gradient');
    ...
    const saveDrawing = () => {
        ...
        link.download = `silindr-chizma-${new Date().getTime()}.png`;
                    ...
                    <div className="fullscreen-whiteboard" ref={canvasContainerRef}>
                        ...
                        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
                            <Canvas shadows>
                                ...
                                <Environment preset="city" />
                                <group scale={renderScale}>
                                    <Silindr3D radius={radius} height={height} tiltAngle={tiltAngle} showWireframe={showWireframe} fillColor={silindrFillColor} />
                                </group>
                                {showGrid && (
                        ...
                        <div className={`whiteboard-toolbar ${isToolbarOpen ? 'open' : ''}`}>
                            ...
                            <div className="toolbar-section fill-section">
                                <span className="fill-label">Silindr:</span>
                                <div className="fill-buttons">
                                    <button className={`fill-btn ${silindrFillColor === 'gradient' ? 'active' : ''}`} onClick={() => setSilindrFillColor('gradient')} title="Gradient">
                                        <span className="gradient-preview"></span>
                                    </button>
                                    {COLOR_PALETTE.slice(0, 5).map(color => (
                                        <button key={`fill-${color}`} className={`fill-btn ${silindrFillColor === color ? 'active' : ''}`} style={{ backgroundColor: color }} onClick={() => setSilindrFillColor(color)} />
                                    ))}
                                </div>
                            </div>
                            <div className="toolbar-divider" />
